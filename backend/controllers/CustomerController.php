...
class CustomerController
{
    ...
    // Get all customers (created by this owner)
    public function getAll()
    {
        $this->authenticate();
        $this->checkOwner();

        $query = "SELECT * FROM customers WHERE created_by = :created_by ORDER BY created_at DESC";
    ...
    public function getOne($id)
    {
        ...
        $query = "SELECT * FROM customers WHERE id = :id AND created_by = :created_by";
        $stmt = $this->db->prepare($query);
    ...
    public function create()
    {
        ...
        error_log("Customer Create Data: " . print_r($data, true));

        if (!empty($data->first_name) && !empty($data->last_name)) {
            $query = "INSERT INTO customers (first_name, last_name, whatsapp_number, pan_card_number, 
                      personal_bank_account_name, personal_bank_name, personal_ifsc_code, personal_bank_id, created_by) 
                      VALUES (:first_name, :last_name, :whatsapp_number, :pan_card_number, 
                      :personal_bank_account_name, :personal_bank_name, :personal_ifsc_code, :personal_bank_id, :created_by)";
            ...
            if ($stmt->execute()) {
                ...
                echo json_encode(array("message" => "Customer created.", "id" => $this->db->lastInsertId()));
            } else {
            ...
            echo json_encode(array("message" => "Incomplete data. First and Last Name required."));
        }
    }
    ...
    public function update($id)
    {
        ...
        $data = json_decode(file_get_contents("php://input"));
        error_log("Customer Update Data ID $id: " . print_r($data, true));

        // Verify ownership
        $checkQuery = "SELECT id FROM customers WHERE id = :id AND created_by = :created_by";
        ...
        if ($checkStmt->rowCount() == 0) {
            http_response_code(404);
            echo json_encode(["message" => "Customer not found or access denied."]);
            return;
        }
        ...
        $whatsapp_number = isset($data->whatsapp_number) ? htmlspecialchars(strip_tags($data->whatsapp_number)) : NULL;
        $pan_card_number = isset($data->pan_card_number) ? htmlspecialchars(strip_tags($data->pan_card_number)) : NULL;
        $personal_bank_account_name = isset($data->personal_bank_account_name) ? htmlspecialchars(strip_tags($data->personal_bank_account_name)) : NULL;
        $personal_bank_name = isset($data->personal_bank_name) ? htmlspecialchars(strip_tags($data->personal_bank_name)) : NULL;
        $personal_ifsc_code = isset($data->personal_ifsc_code) ? htmlspecialchars(strip_tags($data->personal_ifsc_code)) : NULL;
        $personal_bank_id = isset($data->personal_bank_id) ? htmlspecialchars(strip_tags($data->personal_bank_id)) : NULL;
    ...
    public function delete($id)
    {
        ...
        $query = "DELETE FROM customers WHERE id = :id AND created_by = :created_by";
        $stmt = $this->db->prepare($query);
        $stmt->bindParam(":id", $id);
        $stmt->bindParam(":created_by", $this->user_id);
        ...
        if ($stmt->execute()) {
            if ($stmt->rowCount() > 0) {
                ...
                http_response_code(404);
                echo json_encode(array("message" => "Customer not found or access denied."));
            }
        } else {
            http_response_code(503);
